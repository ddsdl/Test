...
# 리스트.insert(인덱스, 요소) -> 리스트 중 특성 인덱스에 요소를 삽입
names.insert(1, '이준호')
print(names)

# 리스트.pop(인덱스) 꺼내기 -> 리스트 중 특성 인덱스에 존재하는 요소를 꺼낸다. 
# (인덱스의 셍략하면 맨 뒤 요소를 꺼낸다)
popped_name = names.pop(2)
print("내가 꺼낸 요소는 --->", popped_name)
print(names)

# 요소 in 리스트 -> 리스트에 해당 요소의 존재 여부를 반환한다.
inPresent = '연제헌' in names
print(inPresent)
...
# 리스트.count(요소) -> 리스트 내 같은값을 가진 요소의 개수를 반환한다.
names.append('이준호')
names.append('이준호')
names.append('이준호')
print(names)
